...
# Find the top 3 largest gamma with strike prices
top3 = sorted(zip(fdfAgg['TotalGamma'].to_numpy(), strikes), reverse=True)[:3]
top1_LG = '1st LG: '
top2_LG = '2nd LG: '
top3_LG = '3rd LG: '

total_gex =  'Total GEX: $' +str("{:.2f}".format(fdfAgg['TotalGEX'].sum())) + ' Billion'
if len(top3) == 3:
    top1_LG += str(top3[0][1]) + ',  ' + str("{:,.0f}".format(top3[0][0] * (10**9)))
    top2_LG += str(top3[1][1]) + ',  ' + str("{:,.0f}".format(top3[1][0] * (10**9)))
    top3_LG += str(top3[2][1]) + ',  ' + str("{:,.0f}".format(top3[2][0] * (10**9)))
...
# sortedTotalGamma = sorted(totalGamma)
# print(sortedTotalGamma)
...
print('Spot Price: ', str("{:,.2f}".format(spotPrice)))
